...
function loadExperience(data, ExperienceName) {
  ...
  if (storypointsdata.splashBackgroundColor && storypointsdata.Opacity) {
    var rgbformat = hexToRgb(
      storypointsdata.splashBackgroundColor,
      storypointsdata.Opacity
    );
  ...
  if (storypointsdata.splashHeaderColor) {
    document.getElementById("titleText").style.color =
      storypointsdata.splashHeaderColor;
  }
  document.getElementById("titleText").innerHTML = ExperienceName;
  // document.getElementById("instruction").innerHTML =
  //   storypointsdata.splash_instruction;
  
  if (isMobile && storypointsdata.splash_android_instruction!=undefined) {
		document.getElementById('instruction').innerHTML = storypointsdata.splash_android_instruction;
	  } else {
  ...
  for (i = 1; i <= storypointsdata.actionPoints.length; i++) {
    ...
    background.setAttribute(
      "set-sky",
      storypointsdata.actionPoints[i - 1].mediatype + " " + i
    );
    background.setAttribute("visible", "false");
    background.setAttribute("transparent", "true");
    var posbackground = storypointsdata.actionPoints[
      i - 1
    ...
    newTeleport.setAttribute("material","src",'/act/v0.1/sequence/assets/images/icons/square-'+i+'.png');
    ...
    if (
      storypointsdata.actionPoints[i - 1].actionPointVisibility ==
      "Always Visible"
    ) {
  ...
  init() {
    this.el.addEventListener("mouseenter", () => {
      if (experienceStarted) {
        ...
        if (
          storypointsdata.actionPoints[mediatype[1] - 1]
            .actionPointVisibility == "Visible while gaze over object"
        ) {
    ...
    this.el.addEventListener("click", () => {
      if (experienceStarted == true) {
        ...
        var mediatype = this.data.split(" ");
        var posvid = storypointsdata.actionPoints[
          parseInt(mediatype[1]) - 1
        ].actionPointPosition.split(" ");
        var pos = newPos(posvid[0], posvid[1], posvid[2]);

        if (
          storypointsdata.actionPoints[mediatype[1] - 1].actionSize === "small"
        ) {
          background.setAttribute(
          ...
          storypointsdata.actionPoints[mediatype[1] - 1].actionSize === "medium"
        ) {
          background.setAttribute(
            "animation__startAnimation",
        ...
        switch (mediatype[0]) {
          case "V":
            ...
            var videoSource =
              storypointsdata.actionPoints[mediatype[1] - 1].actionPointVideo;
          ...
          case "VT":
            ...
            if (
              storypointsdata.actionPoints[parseInt(mediatype[1]) - 1]
                .actionPointBgColor
            ) {
            ...
            var postext = storypointsdata.actionPoints[
              parseInt(mediatype[1]) - 1
            ].actionPointPosition.split(" ");
        ...
        clicked = true;
        if (
          storypointsdata.actionPoints[parseInt(mediatype[1]) - 1].markarTitle
        )
          obj =
            storypointsdata.actionPoints[parseInt(mediatype[1]) - 1]
              .markarTitle;
      ...
      if (experienceStarted && clicked) {
        ...
        switch (mediatype[0]) {
          ...
          case "A":
            ...
            if (nosafari) audio.setAttribute("sound", "volume:0;");
            else audio.components.sound.stopSound();
