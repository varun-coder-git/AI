...
AFRAME.registerComponent('mobile', {
    init: function () {
       if( (AFRAME.utils.device.isMobile()) || (!AFRAME.utils.isMobile() && AFRAME.utils.checkHeadsetConnected()) ){
	...
	init: function () {
		...
		this.el.addEventListener('click', function() {
            if(startMovement){
                ...
                var cameraAnimetion;
                var cameraEntity = document.querySelector('#rig');
                ...
                // }
                prev = document.getElementById("rig").getAttribute('position');
                var x = parseInt(prev.x);
                var y = parseInt(prev.y);
                 var z = parseInt(prev.z);
                 if(z ==1)
                var z = 0;
                // alert (x+" " +y+ " " +0);
                cameraEntity.setAttribute('animation__yoyo','property: position; dur: 2000; easing: linear; from:'+x+" " +y+ " " +z +' ;to: '+position.x+" 0 "+position.z);
                   

                // cameraEntity.setAttribute('position', +position.x+" 0 "+position.z)
                // alert('hotspot click ')       
            }
        });
    }
