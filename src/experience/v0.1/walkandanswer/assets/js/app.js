...
function loadExperience(data, ExperienceName) {
	...
	// document.getElementById('instruction').innerHTML = ((data.splash_instruction) ? data.splash_instruction : '');
	if (isMobile) {
	...
	document.getElementById('titleDescription').innerHTML = ((data.launch_text) ? data.launch_text : '');
...
AFRAME.registerComponent('next', {
	init: function () {
		this.el.addEventListener('click', () => {
			if (experienceStarted === true) {
				var Option = "";
				questionJsonArray.Options.forEach(index => {
					if (index.attempted) {
						Option += index.option + ",";
					}
				});
				...
				params.Object = "question '" + questionJsonArray.Question + "' with options'" + Option + "'";
				params.Sentence = params.Actor + " " + params.Verb + " " + params.Object;
				callToTacking.callToApi(params);
				...
				if (xapiEnable) {
					...
					sendQuestionAnswerToXapi("Answered", "answered", questionJsonArray.Question, timestamp + "", questionJsonArray.Options, "choice", "answerKey", Option, true);
				}
				activeScreen = walkArroundJson[parseInt(activeScreen.teleports[answerid.split("-")[2]].teleportToPlaceIndex)];
...
function addHotspot(teleport, index) {
	if (teleport.teleportToPlaceIndex != null && teleport.teleportToPlaceIndex != undefined) {
		...
		newHotspot.setAttribute('opacity', teleport.teleportVisibility + 0.3);
...
function addInfoHotspot(infobox, index) {
	...
	infoboxOverlay.setAttribute('sky', '');
	...
	init: function () {
		...
		this.el.addEventListener('mouseleave', () => {
			if (experienceStarted === true) {
				var cur = document.querySelector('#cursor');
				cur.emit("startAnimantion");
				if (this.el.getAttribute('id').split("-")[1] != "infohotspot") {
...
function sendToXapi(verb, urlVerb, objectText, ObjectUrlName) {
	sendVerboseToXapi(verb, "http://adlnet.gov/expapi/verbs/" + urlVerb, objectText, "http://adlnet.gov/expapi/activities/TourSurvey", "http://id.tincanapi.com/activity/experizer-template/" + ObjectUrlName.toLowerCase(), "http://adlnet.gov/expapi/activities/course", "Activity");
}
	...
	init: function () {
		this.el.addEventListener('click', () => {
			if (experienceStarted === true) {
				...
				var placeIndex = Object.keys(walkArroundJson).find(key => walkArroundJson[key].placeIndex === parseInt(activeScreen.teleports[this.el.getAttribute('id').split("-")[2]].teleportToPlaceIndex));
				activeScreen = walkArroundJson[parseInt(placeIndex)];

				//activeScreen = walkArroundJson[parseInt(activeScreen.teleports[this.el.getAttribute('id').split("-")[2]].teleportToPlaceIndex)];
				params.Verb = "Moved  to";
				params.Object = activeScreen.placeName;
				params.Sentence = params.Actor + " " + params.Verb + " " + params.Object;
				callToTacking.callToApi(params);
