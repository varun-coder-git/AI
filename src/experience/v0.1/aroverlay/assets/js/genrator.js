...
async function generateMarkerImageFile(experienceJSON) {
  var mCount=0;
  for (i = 0; i < experienceJSON.actionMarkers.length; i++) {
    var strURL =experienceJSON.actionMarkers[i].markerPattern;
    imageName=experienceJSON.actionMarkers[i].markerPattern;
 ...
 if(experienceJSON.actionMarkers.length<1){
  popSnackbar("warning", "Add atleast 1 Marker and customize it.");
 }
 for (j = 0; j < experienceJSON.actionMarkers.length; j++) {
   if(experienceJSON.actionMarkers[j].markerPattern!=undefined && experienceJSON.actionMarkers[j].markerPattern!=""){
     mCount++;
   }
   if(j==experienceJSON.actionMarkers.length-1  && mCount<1){
