...
function objectImage(image){
  ...
  $("#markerInspectAsset").attr("src", image);
}
...
function toggleSideNav(sidenavId) {
  ...
  if(sidenavId=="storyPointSidenav"){
    ...
    for (var i = 1; i < experianceJSON.actionPoints.length; i++) {
...
function addNewHotspot(size, type) {
  ...
  if (experianceJSON.actionPoints.length >= 10) {
...
function augmentedInspectGenerator(
  ...
  experianceJSON.actionPoints.forEach(augmentedInpsect => {
    ...
    // settings
    var markerTiles =
      " <div id='displayMarkerImageDiv-" +
      augmentedInspectIndex +
      "' class='col-xl-1 col-lg-1 col-md-1 col-sm-1 footer-col-1-style' ><img id='displayMarkerImage-" +
      augmentedInspectIndex +
      "' class='footer-img-style  marker-image-display' style='background-color: white;' src='' alt='marker image'  onclick='editHotspot(" +
      augmentedInspectIndex +
      ");'/></div>";

    $("#tile-row").append(markerTiles);
    ...
    var augmentedInspectAccordion =
      ...
      augmentedInspectIndex +
      "&#39;' onmouseenter='mouseEnter(&#39;You can delete the image from your experience by simply hitting this button.&#39;);' onmouseleave='mouseLeave();'></span><h3 class='mb-0'><button class='btn btn-link btn-decor' onmouseenter='mouseEnter(&#39;Markers help you overlay Media over the Area images. Configure the markers on the screen using this option.&#39;);' onmouseleave='mouseLeave()' onclick='bindMarkerAndOverlayedAsset(" +
      augmentedInspectIndex +
      ")' data-toggle='collapse' data-target='#collapse-storypoint-" +
      augmentedInspectIndex +
      ...
      augmentedInspectIndex +
      "' class='collapse back-color' aria-labelledby='headingTwo' data-parent='#augmentedInpsect-accordion'><div class='card-body'><div class='form-group'><lable for='markerTitleID-" +
      augmentedInpsect.actionId +
      ...
      "' value='" +
      markerValue +
      "'></div><div class='form-group mb-2 set-image-size ' id='SelectedImgID-" +
      augmentedInpsect.actionId +
      "'><lable for='markerImageID-" +
      augmentedInpsect.actionId +
      "' class='font_color'>Marker Image:</lable><img src='" +
      augmentedInpsect.image_path +
      "' class='set-image-size'  style='background-color: white;' id='imgID-" +
      augmentedInpsect.actionId +
      "'></div><div class='form-group custom-file' id='BrowseImgID-" +
      augmentedInpsect.actionId +
      ...
      "'>" +
      imagename +
      "</span></label></div><label class='aspect-ratio-style ' for='startquizbtn' style='font-size: 12px'>Note: Recommended aspect ratio 1:1</label><div class='form-group '><lable for='typeDropDownID-" +
      augmentedInpsect.actionId +
    ...
    if (augmentedInpsect.image_path) {
      ...
      $("#imgID-"+ augmentedInpsect.actionId).attr("src", "assets/images/default.jpg");
      $("#displayMarkerImage-" + augmentedInspectIndex).attr(
        "src",
    ...
    if (augmentedInpsect.obj_image) {
      ...
      $("#markerInspectAsset").attr("src", "assets/images/default.jpg");
    }
...
function bindMarkerAndOverlayedAsset(id) {
  for (var i = 0; i < experianceJSON.actionPoints.length; i++) {
    if (experianceJSON.actionPoints[i].actionId == id) {
      ...
      $("#markerInspectAsset").attr("src", modelImage);
      $("#markerImageDisplay").attr("src", assetPath);
...
function openSideNav(sidenavId) {
  ...
  [].forEach.call(sidenavList, function(navigation) {
    if (navigation.classList.contains("active") && navigation.id != sidenavId)
      navigation.classList.remove("active");
  $("#collapse-storypoint-" + (augmentedInspectIndex-1)).addClass("show");
  });
