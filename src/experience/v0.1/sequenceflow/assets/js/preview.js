...
function initializePreview(experienceJSON, ExperienceName) {
  ...
  sequenceData = JSON.parse(experienceJSON);
  ...
  submitquiz.setAttribute("class","clickable");
  submitquiz.setAttribute("show-score","");
...
AFRAME.registerComponent('submit-close', {
  ...
  init() {
    ...
    this.el.addEventListener('click', () => {
      ...
      var mainInfo = document.getElementById('mainInfo');
      mainInfo.setAttribute("animation__endAnimation","property: scale;to:0 0 0;from:1.5 1.5 1.5; dur: 1000;enabled:true;");
      setTimeout(function() {
...
function scorecard() {
  ...
  setTimeout(function () {
    ...
    // panelScreen.setAttribute('visible', 'false');
    // panelScreen.setAttribute('position', '2000 2000 2000');
