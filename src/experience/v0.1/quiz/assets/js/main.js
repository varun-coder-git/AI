...
function initializeCustomization(experienceToCustomize) {
  ...
  if (quizData.launch_text) {
    document.getElementsByClassName("lunchScreenText").value = quizData.launch_text;
  ...
  if (quizData.splash_instruction) {
    document.getElementsByClassName("instructionSetForDesktop").value = quizData.splash_instruction;
...
AFRAME.registerComponent('mouseclick', {
  init: function () {
    this.el.addEventListener('click', function () {
      ...
      if (imageOrder[1] == '0')
        $("#" + clickedImageId[0] + "-wall-logoLabel").addClass('background-color-forestgreen');
      else if (imageOrder[1] == '1')
        $("#" + clickedImageId[0] + "-wall-image1Label").addClass('background-color-forestgreen');
      else if (imageOrder[1] == '2')
        $("#" + clickedImageId[0] + "-wall-image2Label").addClass('background-color-forestgreen');
    ...
    if (quizData.questionData.length < 20) {
      ...
      if (quizData.questionData.length > 0) {
        ...
        textview.setAttribute(
          "text", "value:" + quizData.questionData[questionLength - 1].Question
        );
        for (var j = 0; j < 4; j++) {
          var option = document.getElementById("option-" + j);
          option.setAttribute("visible", "false");
        }
        for (var j = 0; j < quizData.questionData[len].Options.length; j++) {
          var option = document.getElementById("option-" + j);
          option.setAttribute("visible", "true");
          option.setAttribute(
            "text", "value:" + quizData.questionData[len].Options[j].option
          );
        }
      }
      $(".collapse").removeClass("show");
      //	$("#collapse"+questionLength).addClass("show");
      $("#addQuestionDivCollapse").addClass("show");
...
function editQuestion(id) {
  ...
  for (i = 0; i < data.length; i++) {
    if (data[i].Id == id) {
      $('#displayquestiontitle').html("Question " + (id));
...
function deleteQuestionAfterConfirmation() {
  ...
  if (quizData.questionData.length > 0) {
    ...
    for (var j = 0; j < quizData.questionData[questionLength - 1].Options.length; j++) {
      var option1 = document.getElementById('option-' + j);
      option1.setAttribute('text', 'value:' + quizData.questionData[questionLength - 1].Options[j].option);
    }
  } else {
    textview = document.getElementById('questionText');
    //textview.setAttribute('text','value:'+'Question text appears here');
    textview.setAttribute('visible', 'false');
    for (var j = 0; j < 4; j++) {
      var option1 = document.getElementById('option-' + j);
      //option1.setAttribute('text','value:'+  'Option appears here');
...
function saveQuestion() {
  ...
  textview.setAttribute("visible", "true");
  textview.setAttribute(
    "text", "value:" + quizData.questionData[data.length - 1].Question
  );
  for (var j = 0; j < 4; j++) {
    var option = document.getElementById("option-" + j);
    option.setAttribute("visible", "false");
  }
  for (var j = 0; j < quizData.questionData[data.length - 1].Options.length; j++) {
    var option = document.getElementById("option-" + j);
    option.setAttribute("visible", "true");
    option.setAttribute(
      "text", "value:" + quizData.questionData[data.length - 1].Options[j].option
    );
  }
  ...
  for (i = 0; i < data.length; i++) {
    ...
    var accordionData = '<div class="card questionAccordion" id="divGroup' + bind_questionNum + '"> <div class="card-header" id="heading' + bind_questionNum + '"> <h3 class="mb-0"> <button id="que' + bind_questionNum + '" class="btn btn-link" data-toggle="collapse" data-target="#collapse' + bind_questionNum + '" aria-expanded="true" aria-controls="collapse' + bind_questionNum + '" onclick="setQuestionOnScreen(' +
      bind_questionNum +
...
function getAssetPath(assetPath) {
  ...
  if (elementToUpdate == 'Splash') {
    ...
    if (assettype == 'Audio') {
      ...
      var path = returnAssetName(assetPath);
...
function bindData(input, wall, order) {
  ...
  if (input != null) {
    ...
    if (wall == "south") {
      if (order == 0) {
        if (galleryImageName == "" || galleryImageName == undefined) {
          ...
          prevSouthLogo = galleryImageName;
          quizData.branding_south_wall["Logo"] = galleryImageName;
          var lbl = document.getElementById("south-wall-logoLabel");
          var path = returnAssetName(galleryImageName);
          lbl.children[0].innerText = path;
        ...
        if (galleryImageName == "" || galleryImageName == undefined) {
          ...
          prevSouthImage = galleryImageName;
          quizData.branding_south_wall["image2"] = galleryImageName;
          var lbl = document.getElementById("south-wall-image2Label");
          var path = returnAssetName(galleryImageName);
          lbl.children[0].innerText = path;
      ...
      if (order == 0) {
        if (galleryImageName == "" || galleryImageName == undefined) {
          galleryImageName = "assets/images/default.jpg";
          quizData.branding_west_wall["Logo"] = galleryImageName;
          var lbl = document.getElementById("west-wall-logoLabel");
          var path = returnAssetName(galleryImageName);
          lbl.children[0].innerText = path;
          changedgallery.setAttribute("src", galleryImageName);
        } else {
          prevWestImage = galleryImageName;
          quizData.branding_west_wall["Logo"] = galleryImageName;
          var lbl = document.getElementById("west-wall-logoLabel");
          var path = returnAssetName(galleryImageName);
          lbl.children[0].innerText = path;
      ...
      if (order == 0) {
        if (galleryImageName == "" || galleryImageName == undefined) {
          galleryImageName = "assets/images/default.jpg";
          quizData.branding_east_wall["Logo"] = galleryImageName;
          var lbl = document.getElementById("east-wall-logoLabel");
          var path = returnAssetName(galleryImageName);
          lbl.children[0].innerText = path;
          changedgallery.setAttribute("src", galleryImageName);
        } else {
          prevEastImage = galleryImageName;
          quizData.branding_east_wall["Logo"] = galleryImageName;
          var lbl = document.getElementById("east-wall-logoLabel");
          var path = returnAssetName(galleryImageName);
          lbl.children[0].innerText = path;
  ...
  if (input == "") {
    if (wall == "south") {
      if (order == 0) {
        if (prevSouthLogo == "" || prevSouthLogo == undefined) {
          prevSouthLogo = "assets/images/default.jpg";
          quizData.branding_south_wall["Logo"] = prevSouthLogo;
          var lbl = document.getElementById("south-wall-logoLabel");
          var path = returnAssetName(prevSouthLogo);
          lbl.children[0].innerText = path;
          changedgallery.setAttribute("src", prevSouthLogo);
        } else {
          ...
          gallery_entity.setAttribute("src", prevSouthLogo);
        }
      } else if (order == 2) {
        if (prevSouthImage == "" || prevSouthImage == undefined) {
          prevSouthImage = "assets/images/default.jpg";
          quizData.branding_south_wall["Logo"] = prevSouthImage;
          var lbl = document.getElementById("south-wall-image2Label");
          var path = returnAssetName(prevSouthImage);
          lbl.children[0].innerText = path;
          changedgallery.setAttribute("src", prevSouthImage);
        } else {
          var gallery_entity = document.querySelector("#south-wall-entity2");
          quizData.branding_south_wall["image2"] = prevSouthImage;
          var lbl = document.getElementById("south-wall-image2Label");
          var path = returnAssetName(prevSouthImage);
          lbl.children[0].innerText = path;
          gallery_entity.setAttribute("src", "");
          gallery_entity.setAttribute("src", prevSouthImage);
        }
      }
    } else if (wall == "east") {
      if (order == 0) {
        if (prevEastImage == "" || prevEastImage == undefined) {
          prevEastImage = "assets/images/default.jpg";
          quizData.branding_east_wall["Logo"] = prevEastImage;
          var lbl = document.getElementById("east-wall-logoLabel");
          var path = returnAssetName(prevEastImage);
          lbl.children[0].innerText = path;
          changedgallery.setAttribute("src", prevEastImage);
        } else {
          ...
          lbl.children[0].innerText = path;
          gallery_entity.setAttribute("src", "");
          gallery_entity.setAttribute("src", prevEastImage);
        }
      }
    } else if (wall == "west") {
      if (order == 0) {
        if (prevWestImage == "" || prevWestImage == undefined) {
          prevWestImage = "assets/images/default.jpg";
          quizData.branding_west_wall["Logo"] = prevWestImage;
          var lbl = document.getElementById("west-wall-logoLabel");
          var path = returnAssetName(prevWestImage);
          lbl.children[0].innerText = path;
          changedgallery.setAttribute("src", prevWestImage);
        } else {
          ...
          lbl.children[0].innerText = path;
          gallery_entity.setAttribute("src", "");
          gallery_entity.setAttribute("src", prevWestImage);
        }
      }
    }
  }
...
function setQuiz() {
  ...
  var gallery_entity = document.querySelector("#east-wall-entity0");
  if (
    quizData.branding_east_wall.Logo != "" &&
    quizData.branding_east_wall.Logo != undefined
  ) {
    gallery_entity.setAttribute("src", quizData.branding_east_wall.Logo);
    gallery_entity.setAttribute("crossorigin", "anonymous");
    var lbl = document.getElementById("east-wall-logoLabel");
    var path = returnAssetName(quizData.branding_east_wall.Logo);
    lbl.children[0].innerText = path;
  } else {
  ...
  var gallery_entity = document.querySelector("#west-wall-entity0");
  if (
    quizData.branding_west_wall.Logo != "" &&
    quizData.branding_west_wall.Logo != undefined
  ) {
    gallery_entity.setAttribute("src", quizData.branding_west_wall.Logo);
    gallery_entity.setAttribute("crossorigin", "anonymous");
    var lbl = document.getElementById("west-wall-logoLabel");
    var path = returnAssetName(quizData.branding_west_wall.Logo);
    lbl.children[0].innerText = path;
    // westWallImagePath = quizData.branding_west_wall.Logo;
  } else {
    gallery_entity.setAttribute("src", defaultImage);
    gallery_entity.setAttribute("crossorigin", "anonymous");
    var lbl = document.getElementById("west-wall-logoLabel");
    ...
    // quizData.branding_west_wall.Logo = westWallImagePath;
  }
  ...
  $('input:radio[name="right"]').filter('[value="' + quizData.Show_right_answer + '"]').attr('checked', true);
  $('input:radio[name="score"]').filter('[value="' + quizData.Show_score_at_end + '"]').attr('checked', true);
  ...
  for (i = 0; i < data.length; i++) {
    ...
    var accordionData = '<div class="card questionAccordion" id="divGroup' + bind_questionNum + '"> <div class="card-header" id="heading' + bind_questionNum + '"> <h3 class="mb-0"> <button class="btn btn-link" data-toggle="collapse" data-target="#collapse' + bind_questionNum + '" aria-expanded="true" aria-controls="collapse' + bind_questionNum + '" onclick="setQuestionOnScreen(' +
      bind_questionNum + ')" onclick="setQuestionOnScreen(' +
      bind_questionNum + ')"> <i class="fa" aria-hidden="true"></i> Question <span>' + bind_questionNum + '</span> </button> </h3> </div> <div id="collapse' + bind_questionNum + '" class="collapse" aria-labelledby="heading' + bind_questionNum + '" data-parent="#accordion"> <div class="card-body"> <div class="row ' + dynamic_panel + '"><div class="col-12">' + bind_questionText + '</div>' + optionData + '</div> ' + answer_data + feedback_data + '<div class="row"><div class="col-12 actionDiv"><button onmouseenter="mouseEnter(\'Edit the question using this button.\');" onmouseleave="mouseLeave()" type="button" class="btn btn-success btn-margin-right" onclick="editQuestion(' + data[i].Id + ')">Edit</button><button onmouseenter="mouseEnter(\'In case if you wish to roll-back the changes without saving them, press this button.\');" onmouseleave="mouseLeave()" type="button" class="btn btn-danger" onclick="deleteQuestion(' + data[i].Id + ')" data-toggle="modal" data-target="#deleteConfirmationModal">Delete</button></div></div></div></div></div>';
    $("#accordion").append(accordionData);
  }
  questionCount = data.length;
  $("#questionTextarea").val("");
...
function getExperienceToSave() {
  ...
  if (splashImagePath)
    quizData.splash_image = splashImagePath;
...
function setQuestionOnScreen(questionNumber) {
  ...
  for (var index = 0; index < questionLength; index++) {
    if (index == decQuestionNumber) {
      ...
      //   var lblWestWallImage = document.getElementById("eastWallImageLable");
      //   lblWestWallImage.children[0].innerText = returnAssetName(
