...
function bindAssetPath(assetPath) {
  if (elementToUpdate === 'Splash') {
    ...
    if(assetPath != ""){
      ...
      var placeElementIndex = $("#place-sky-image-" + selectedElementIndex);
      placeElementIndex[0].children[0].innerText = previousPlaceImagePath.split("/").pop();
      $("#place-image-path-" + selectedElementIndex).text(previousPlaceImagePath);
      $("#sky-image-preview-" + selectedElementIndex).attr("src", previousPlaceImagePath);
      addPlaceSkyToJSON(selectedElementIndex);
    }
  }
    ...
    if(assetPath != ""){
      ...
      $("#infobox-image-path-" + selectedElementIndex).text(previousPathImage);
      $("#infobox-image-preview-" + selectedElementIndex).attr(
        "src",
        previousPathImage
      );
    ...
    if(assetPath != ""){
      ...
      var placeElementIndex = $("#place-sky-audio-" + selectedElementIndex);
      placeElementIndex[0].children[0].innerText = previousPathAudioPlace.split("/").pop();
      $("#place-audio-path-" + selectedElementIndex).text(previousPathAudioPlace);
      addPlaceSkyToJSON(selectedElementIndex);
      editPlace();
    }
    ...
    if(assetPath != ""){
      ...
      $("#infobox-audio-path-" + selectedElementIndex).text(previousPathAudioBanner);
      $("#infobox-audio-preview-" + selectedElementIndex).attr("src", previousPathAudioBanner);
      saveInfoboxAudioOnChange(selectedElementIndex);
    }
  }
}
...
function addNewTeleport(flag) {
  ...
  selectedHotspot =  $("a-sphere#"+selectedNewTeleportID)[0];
  toggleToolbarButtons(selectedHotspot);
  
    //$('#teleport-accordion').find('.collapse').collapse('hide');
    //$('#collapse-' +teleportIndex).addClass("show");
}
...
function addPlaceSkyToJSON(placeIndex) {
  for (i = 0; i < nodes.length; i++) {
    if (nodes[i].placeIndex === placeIndex) {
      selectedNode = nodes[i];
      // nodes[index].placeName = $("#place-name-input-" + placeIndex).val();
      nodes[i].placeImageName = $("#place-image-path-" + selectedNode.placeIndex).text().split('/').pop();
...
function saveInfoboxImageOnChange(selectedInfobox) {
  if (selectedNode != null && selectedNode != undefined && selectedInfobox != null && selectedInfobox != undefined) {
    nodes[nodes.indexOf(selectedNode)].infoboxes[selectedInfobox].imageName = $('#infobox-image-path-' + selectedInfobox).text().split('/').pop();
...
function setTeleportSelected(teleportIndex) {
  if (selectedHotspot != selectedNode.teleports[teleportIndex] || selectedHotspot == null || selectedHotspot == undefined) {
    selectedHotspot =  selectedHotspot =  $("a-sphere#teleport-"+teleportIndex)[0];
    $('#toggleMapEditor').removeClass('disabled');
    $("#graph").addClass("hidden");
    $("#scene").removeClass("hidden");
    toggleMapEditorLabel();
