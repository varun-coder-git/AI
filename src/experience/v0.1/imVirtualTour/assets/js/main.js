    ...
    this.el.addEventListener("click", function () {
      if (experienceStarted === true) {
        if (this.getAttribute("id").split("-")[1] == "hotspot") {
          ...
          params.Object =
            activeScreen.teleports[this.getAttribute("id").split("-")[2]].name;
          params.Sentence =
          ...
          telportName =
            activeScreen.teleports[this.getAttribute("id").split("-")[2]].name;
          if (xapiEnable) {
            var object = activeScreen.teleports[
              this.getAttribute("id").split("-")[2]
            ].name.replace(/\s/g, "");
            sendToXapi(
              "Focused",
              "focused",
          ...
          for (let i = 0; i < imVirtualTourJson.length; i++) {
            if (
              imVirtualTourJson[i].placeIndex ==
              activeScreen.teleports[this.getAttribute("id").split("-")[2]]
                .teleportToPlaceIndex
            ) {
              activeScreen = imVirtualTourJson[i];
...
function sendToXapi(verb, urlVerb, objectText, ObjectUrlName) {
  sendVerboseToXapi(
    ...
    objectText,
    "http://adlnet.gov/expapi/activities/VirtualTour",
...
function loadExperience(data, ExperienceName) {
  ...
  loggedin = true;
  imVirtualTourJson = data.Nodes;
  activeScreen = imVirtualTourJson[0];
...
function generateRoom(activeScreen) {
  ...
  skyImage.setAttribute(
    "smallSphereAnimationStart",
    "property: geometry.thetaLength;from:0;to:360; dur: 2000;fill:forward;enable:true"
  );
...
function addHotspot(teleport, index) {
  if (teleport.teleportToPlaceIndex != null) {
    ...
    hotspotSphereBG.setAttribute(
      "animation",
      "property:scale;from:0.9 0.9 0.9;to:1 1 1;loop:true;enabled:true;dir:alternate;autoplay:true"
    );
...
function addInfoHotspot(teleport, index) {
  ...
  if (teleport.infoBgOpacity) {
    ...
    infoboxPlane1.setAttribute(
      "material",
      "side:double;opacity:0.8;transparent:true"
    );
  }
  ...
  infoboxPlane.setAttribute("position", "-0.104 -9.321 5.126");
  ...
  if (teleport.infoboxType === "Image") {
    ...
    infoboxText.setAttribute(
      "text",
      "width:28;baseline:top;wrapCount:48;lineHeight:50;alphaTest:4.000"
    );
  ...
  infoboxOverlay.setAttribute("scale", "1 1 1");
