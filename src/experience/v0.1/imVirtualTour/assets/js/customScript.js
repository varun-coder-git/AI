...
var splashImagePath;
// var previousPathImage;
// var previousPathVideo;
// var previousPathAudioPlace;
// var previousPathAudioBanner;
// var previousPlaceImagePath;
var mapLable =
  "Open the 360 View for viewing the Active Place. This opens up the 360 Place for your customization. You can layout your Banners and Hotspots on the same.";
...
function initializeCustomization(experienceToCustomize) {
  ...
  if (experianceJSON.splash_instruction) {
    document.getElementsByClassName("instructionSetForDesktop").value =
      experianceJSON.splash_instruction;
  ...
  if (experianceJSON.splashBackgroundColor) {
    ...
    defaultBackground.style.backgroundColor = "#2B2B2B";
    defaultBackground.value = "#2B2B2B";
  }
  ...
  if (experianceJSON.Opacity) {
    ...
    document.getElementById("rangeValue").innerText = displayvalue;
    document.getElementById("splashBackground").style.opacity = experianceJSON.Opacity;
  } else {
    var opacity = document.getElementById("fixOpacity");
...
function setEntryView() {
  if ($("#freezeView").hasClass("disabled")) {
    ...
    $("#freezeView").click(
      toastr.success("Presto! This view is now the Launch View of this Place.")
    );
  }
}
...
function uploadImage(element, id, placeId) {
  if (element === "Place") assettype = "360Image";
  else if (element === "Infobox") assettype = "Image";
...
function uploadVideo(element, id, placeId) {
  assettype = "Video";
  elementToUpdate = element;
  selectedElementIndex = id;
  selectedPlaceToAssign = placeId;
...
function bindAssetPath(assetPath) {
  if (elementToUpdate === "Splash") {
    ...
    if (assettype == "Image") {
      if (assetPath != "") {
        var previousPathImage = assetPath;
        $(
          "#infobox-image-label-" + selectedElementIndex
        )[0].children[0].innerText = assetPath.split("/").pop();
        $("#infobox-image-path-" + selectedElementIndex).text(assetPath);
        $("#infobox-image-preview-" + selectedElementIndex).attr(
          "src",
          assetPath
        );
        ...
        $(
          "#infobox-image-label-" + selectedElementIndex
        )[0].children[0].innerText = previousPathImage.split("/").pop();
        $("#infobox-image-path-" + selectedElementIndex).text(
          previousPathImage
        );
        $("#infobox-image-preview-" + selectedElementIndex).attr(
          "src",
      ...
      if (assetPath != "") {
        var previousPathVideo = assetPath;
        $(
          "#infobox-video-label-" + selectedElementIndex
        )[0].children[0].innerText = assetPath.split("/").pop();
        $("#infobox-video-path-" + selectedElementIndex).text(assetPath);
        $("#infobox-video-preview-" + selectedElementIndex).attr(
          "src",
          assetPath
        );
        saveInfoboxVideoOnChange(selectedElementIndex);
      } else {
        $(
          "#infobox-video-label-" + selectedElementIndex
        )[0].children[0].innerText = previousPathVideo.split("/").pop();
        $("#infobox-video-path-" + selectedElementIndex).text(
          previousPathVideo
        );
        $("#infobox-video-preview-" + selectedElementIndex).attr(
          "src",
  ...
  } else if (elementToUpdate === "placeAudio") {
    if (assetPath != "") {
      var previousPathAudioPlace = assetPath;
      ...
      var placeElementIndex = $("#place-sky-audio-" + selectedElementIndex);
      placeElementIndex[0].children[0].innerText = previousPathAudioPlace
        .split("/")
        .pop();
      $("#place-audio-path-" + selectedElementIndex).text(
        previousPathAudioPlace
      );
      addPlaceSkyToJSON(selectedElementIndex);
      editPlace();
    }
  } else if (elementToUpdate === "InfoboxAudio") {
    if (assetPath != "") {
      var previousPathAudioBanner = assetPath;
      $(
        "#infobox-audio-label-" + selectedElementIndex
      )[0].children[0].innerText = assetPath.split("/").pop();
      $("#infobox-audio-path-" + selectedElementIndex).text(assetPath);
      $("#infobox-audio-preview-" + selectedElementIndex).attr(
        "src",
        assetPath
      );
      ...
      $(
        "#infobox-audio-label-" + selectedElementIndex
      )[0].children[0].innerText = previousPathAudioBanner.split("/").pop();
      $("#infobox-audio-path-" + selectedElementIndex).text(
        previousPathAudioBanner
      );
      $("#infobox-audio-preview-" + selectedElementIndex).attr(
        "src",
        previousPathAudioBanner
    ...
    experianceJSON.Nodes.forEach((place) => {
      ...
      place.teleports.forEach((teleport) => {
        ...
        for (i = 0; i < experianceJSON.Nodes.length; i++) {
          if (
            parseInt(teleport.teleportToPlaceIndex) ===
            experianceJSON.Nodes[i].placeIndex
          )
...
function restart() {
  ...
  g.append("svg:circle")
    ...
    .on("mousedown", (d) => {
      ...
      toggleToolbarPlaceButtons(selectedNode, true);
...
function addNewPlace(flag) {
  ...
  const newPlace = {
    id: ++lastNodeId,
    nodeIndex: lastNodeId,
    placeName: "New place",
    placeIndex: lastNodeId,
    placeSky: "assets/images/DefaultNightSky.jpg",
    placeImageName: "DefaultNightSky.jpg",
    teleports: [],
    infoboxes: [],
  };
...
function removePlace() {
  if (nodes.length == 1) {
    popSnackbar(
      "warning",
      "Atleast one place is required. Can not remove this place."
    );
    return;
  }
...
function toggleToolbarPlaceButtons(selectedPlace, placeSidenavFlag) {
  if (selectedPlace) {
    ...
    $("#teleport-place-name").text(
      "Customize teleports for " + selectedNode.placeName
    );
    $("#infobox-place-name").text(
      "Customize banners for " + selectedNode.placeName
    );
  } else {
...
function addPlaceCards(placeData, newPlaceID) {
  ...
  placeData.forEach((place) => {
    ...
    // var deleteButtonForPlace = "";
    // if(place.placeIndex != 0){
    //   deleteButtonForPlace = "<i class='fa fa-trash float-right trash-icon-position' aria-hidden='true' onmouseenter='mouseEnter(\"Remove this place from this Experience. Remember, by removing this place your Teleports and Banners on the Place also get removed.\");' onmouseleave='mouseLeave()' onclick='setPlaceAndRemove(" + place.placeIndex + ")'></i>";
    // }

    var placeCollapseCard =
      "<div class='card' id='place'><div class='card-header' id='heading-" +
      place.placeIndex +
      "'><h3 class='mb-0'><button class='btn btn-link' data-toggle='collapse' data-target='#collapse-place-" +
      place.placeIndex +
      "' aria-expanded='true' aria-controls='collapse-place-" +
      place.placeIndex +
      "' onclick='setNodeSelected(" +
      index +
      ")'><span id='place-collapse-button-" +
      index +
      "' >" +
      place.placeName +
      "</span></button><i class='fa fa-trash float-right trash-icon-position' aria-hidden='true' onmouseenter='mouseEnter(\"Remove this place from this Experience. Remember, by removing this place your Teleports and Banners on the Place also get removed.\");' onmouseleave='mouseLeave()' onclick='setPlaceAndRemove(" + place.placeIndex + ")'></i></h3></div><div id='collapse-place-" +
      place.placeIndex +
      "' class='collapse' aria-labelledby='heading-" +
      place.placeIndex +
      "' data-parent='#place-accordion'><div class='card-body'><div class='form-group '><label for='place-name-input-" +
      place.placeIndex +
      "'>Name</label><input type='text' placeholder='Place name here.' class='form-control input_box_trasparent' id='place-name-input-" +
      place.placeIndex +
      "' value='" +
      place.placeName +
      "' onchange='saveNameOnBlur(" +
      index +
      "," +
      place.placeIndex +
      ")' maxlength='25' /></div><div class='form-group'><img class='img-fluid cursor-pointer' id='sky-image-preview-" +
      place.placeIndex +
      "'  src=\"" +
      place.placeSky +
      "\" onclick='setSelectedNode(" +
      place.placeIndex +
      ")'></div><div class='form-group custom-file mb-2'><input type='button' class='custom-file-input cursor-pointer' id='custom-file-input-" +
      place.placeIndex +
      "' accept='image/png, image/jpeg' onmouseenter='mouseEnter(\"Place is the 360 view that your Patron sees. You can customize this place with a compatible image through this option. In case if you don&#39;t have a 360 image, you can try uploading a Panoroma OR use the &#39My image is not spherical&#39 checkbox while uploading the image.\");' onmouseleave='mouseLeave()' onclick='changeSkyImage(" +
      place.placeIndex +
      ", " +
      place.placeIndex +
      ")'><label class='custom-file-label' id='place-sky-image-" +
      place.placeIndex +
      "' for='custom-file-input-" +
      place.placeIndex +
      "'><span>" +
      place.placeImageName +
      "</span></label><label class='hidden-label' id='place-image-path-" +
      place.placeIndex +
      "' >" +
      place.placeSky +
      "</label><label class='aspect-ratio-style'>Note: Recommended aspect ratio 2:1</label></div><label for='place-name-input-0' style='margin-bottom: 4%;color:white;margin-top: 5%;'>Background Audio:</label>" +
      "<div class='form-group' id='place-audio-preview-div-" +
      place.placeIndex +
      "'><audio class='audio-width' controls id='place-audio-preview-" +
      place.placeIndex +
      "' src=\"" +
      place.placeAudio +
      "\"></audio></div><div class='form-group custom-file mb-2'><input type='button' class='custom-file-input cursor-pointer' id='custom-file-input-" +
      place.placeIndex +
      "' accept='mp3' onmouseenter='mouseEnter(\"Use this setting to Customize the audio that you would want to play while the Patron is going through the Places.\");' onmouseleave='mouseLeave()' onclick='changeSkyAudio(\"placeAudio\"," +
      place.placeIndex +
      ", " +
      place.placeIndex +
      ")'><label class='custom-file-label' id='place-sky-audio-" +
      place.placeIndex +
      "' for='custom-file-input-" +
      place.placeIndex +
      "'><span>" +
      placeAudioName +
      "</span></label><label class='hidden-label' id='place-audio-path-" +
      place.placeIndex +
      "' >" +
      placeAudio +
      "</label></div><div onmouseenter='mouseEnter(\"Remove the Music/Audio track from your Experience.\");onmouseleave='mouseLeave()' class='remove-audio mt-1'><span onclick='removeSkyAudio(" +
      place.placeIndex +
      ")'>Remove Audio</span></div></div></div></div>";
  ...
  var addTeleportButton =
    '<button onmouseenter="mouseEnter(\'Add a new Place in this Experience. You can later link this place from the Default place with the help of a Teleport.\');" onmouseleave="mouseLeave()" type="button" class="btn btn-success float-right mb-2" onclick="addNewPlace(false)">New Place</button>';
...
function addInfoboxCards(selectedPlace, selectedNewInfoboxID) {
  ...
  if (selectedPlace) {
    $("#infobox-place-name").text(
      "Customize banners for " + selectedPlace.placeName
    );
    var index = 0;
    infoboxIndex = 0;
    selectedPlace.infoboxes.forEach((infobox) => {
      ...
      if (infobox.videoName && infobox.videoPath) {
        infoboxVideo = infobox.videoPath;
        infoboxVideoName = infobox.videoName;
      } else {
        infoboxVideo = "";
        infoboxVideoName = "Choose Video";
      }
      ...
      var infoboxCollapseCard =
        '<div class="card" id="infobox-' +
        infobox.infoboxIndex +
        '"><div class="card-header" id="heading' +
        infobox.infoboxIndex +
        '"><h3 class="mb-0"><button class="btn btn-link" data-toggle="collapse" data-target="#collapse-infobox-' +
        infoboxIndex +
        '" aria-expanded="true" aria-controls="collapse-infobox-' +
        infoboxIndex +
        '" onclick="setInfoboxSelected(' +
        infobox.infoboxIndex +
        ')"><span id="infobox-collapse-button-' +
        index +
        '" >' +
        infobox.infoboxName +
        '</span></button><i class="fa fa-trash float-right trash-icon-position" aria-hidden="true" onmouseenter="mouseEnter(\'Remove the currently selected Banner.\');" onmouseleave="mouseLeave()" onclick="setInfoboxAndRemove(' +
        infobox.infoboxIndex +
        ')"></i></h3></div><div id="collapse-infobox-' +
        infoboxIndex +
        '" class="collapse" aria-labelledby="heading' +
        infobox.infoboxIndex +
        '" data-parent="#infobox-accordion"><div class="card-body"><div class="form-group"><label for="infobox-name-input-' +
        infobox.infoboxIndex +
        '">Name</label><input type="text" placeholder="Infobox name here." class="form-control input_box_trasparent" id="infobox-name-input-' +
        infobox.infoboxIndex +
        '" value="' +
        infobox.infoboxName +
        '" onblur="saveInfoboxNameOnBlur(' +
        index +
        ');" maxlength="25"></div><div class="form-group"><label for="infobox-type-selection-' +
        infobox.infoboxIndex +
        '">Type</label><select class="form-control input_box_trasparent" id="infobox-type-selection-' +
        infobox.infoboxIndex +
        '" onchange="toggleInfoboxType(this.value,' +
        infoboxIndex +
        ');"><option value="Text" id="' +
        infobox.infoboxIndex +
        '-text">Text</option><option value="Image" id="' +
        infobox.infoboxIndex +
        '-image" >Image</option><option value="Audio" id="' +
        infobox.infoboxIndex +
        '-audio">Audio</option><option value="Video" id="' +
        infobox.infoboxIndex +
        '-video">Video</option></option><option value="ImageText" id="' +
        infobox.infoboxIndex +
        '-imagetext">Image with Text</option><option value="AudioText" id="' +
        infobox.infoboxIndex +
        '-audiotext">Audio with Text</option><option value="AudioImage" id="' +
        infobox.infoboxIndex +
        '-audioimage">Audio with Image</option><option value="AudioTextImage" id="' +
        infobox.infoboxIndex +
        '-audiotextimage">Audio with Text and Image</option><option value="VideoText" id="' +
        infobox.infoboxIndex +
        '-videotext">Video with Text</option></select></div><div class="form-group" id="infobox-text-group-' +
        infoboxIndex +
        '"><label for="infobox-text-input-' +
        infobox.infoboxIndex +
        '">Info Text</label><textarea maxlength="460"  placeholder="Info Text here." class="form-control input_box_trasparent" id="infobox-text-input-' +
        infobox.infoboxIndex +
        '" onblur="saveInfoboxTextOnBlur(' +
        index +
        ",'" +
        infobox.infoboxID +
        "')\" >" +
        infobox.text +
        '</textarea></div><div class="form-group" id="infobox-text-color-' +
        infoboxIndex +
        '"><label>Info Text Font Color</label><input class="jscolor pointer " id="infoTextColor-' +
        infobox.infoboxIndex +
        '" onchange="changeInfofont(' +
        index +
        ')" value="" style="width: 100%"></div><div class="form-group" id="infobox-bg-color-' +
        infoboxIndex +
        '"><label>Choose Description Background</label><input class="jscolor pointer " id="infoboxBgColor-' +
        infobox.infoboxIndex +
        '" value="#000"  style="width: 100%" onchange="changeInfoBg(' +
        index +
        ')"></div><div class="form-group" id="infobox-bg-opacity-' +
        infoboxIndex +
        '"><div class="row"><div class="col-10"><label style="color:#fff;width: 100%">Background Opacity (Drag to change):</label></div><div class="col-2"><label id="ImageDescriptionRangeValue' +
        infoboxIndex +
        '">(60%)</label></div></div><div class="slidecontainer"><input type="range" min="0.1" max="1" step="0.001" value="0.6" class="slider" oninput="changeOpacity( this,' +
        infobox.infoboxIndex +
        ')" id="descriptionBgOpacity-' +
        infobox.infoboxIndex +
        '" /></div></div><div class="form-group" id="infobox-image-preview-div-' +
        infoboxIndex +
        '"><img class="img-fluid" id="infobox-image-preview-' +
        infoboxIndex +
        '"  src="' +
        infoboxImage +
        '" ></div><div class="form-group custom-file mb-5" id="infobox-image-group-' +
        infoboxIndex +
        '"><input type="button" class="custom-file-input cursor-pointer" id="infobox-image-input-' +
        infobox.infoboxIndex +
        '"  onclick="addInfoboxImage(' +
        infoboxIndex +
        ", " +
        selectedPlace.placeIndex +
        ')"><label class="custom-file-label" id="infobox-image-label-' +
        infobox.infoboxIndex +
        '" for="infobox-image-input-' +
        infobox.infoboxIndex +
        '"><span>' +
        infoboxImageName +
        '</span></label><label class="hidden-label" id="infobox-image-path-' +
        infobox.infoboxIndex +
        '" >' +
        infoboxImage +
        '</label><label class="aspect-ratio-style">Note: Recommended aspect ratio 16:9</label></div><div class="form-group" id="infobox-video-preview-div-' +
        infoboxIndex +
        '"><video class="img-fluid" controls="controls" id="infobox-video-preview-' +
        infoboxIndex +
        '"  src="' +
        infoboxVideo +
        '" ></div><div class="form-group custom-file mb-5" id="infobox-video-group-' +
        infoboxIndex +
        '"><input type="button" class="custom-file-input cursor-pointer" id="infobox-video-input-' +
        infobox.infoboxIndex +
        '"  onclick="addInfoboxVideo(' +
        infoboxIndex +
        ", " +
        selectedPlace.placeIndex +
        ')"><label class="custom-file-label" id="infobox-video-label-' +
        infobox.infoboxIndex +
        '" for="infobox-video-input-' +
        infobox.infoboxIndex +
        '"><span>' +
        infoboxVideoName +
        '</span></label><label class="hidden-label" id="infobox-video-path-' +
        infobox.infoboxIndex +
        '" >' +
        infoboxVideo +
        '</label></div><div class="form-group" id="infobox-audio-preview-div-' +
        infoboxIndex +
        '"><audio class="audio-width" controls id="infobox-audio-preview-' +
        infoboxIndex +
        '"  src="' +
        infoboxAudio +
        '" ></audio></div><div class="form-group custom-file mb-2" id="infobox-audio-group-' +
        infoboxIndex +
        '"><input type="button" class="custom-file-input cursor-pointer" id="infobox-audio-input-' +
        infobox.infoboxIndex +
        '"  onclick="addInfoboxAudio(' +
        infoboxIndex +
        ", " +
        selectedPlace.placeIndex +
        ')"><label class="custom-file-label" id="infobox-audio-label-' +
        infobox.infoboxIndex +
        '" for="infobox-audio-input-' +
        infobox.infoboxIndex +
        '"><span>' +
        infoboxAudioName +
        '</span></label><label class="hidden-label" id="infobox-audio-path-' +
        infobox.infoboxIndex +
        '" >' +
        infoboxAudio +
        '</label></div><div class="form-group"><label for="infobox-size-input-' +
        infoboxIndex +
        '">Infobox Size</label><input type="range" min="1" max="10" step="0.01" value="' +
        parseInt(infobox.infoboxSize) +
        '" class="slider" id="infobox-size-input-' +
        infobox.infoboxIndex +
        '" oninput="resizeInfobox(' +
        infobox.infoboxIndex +
        ');"></div><div class="form-group"><label for="infobox-opacity-input-' +
        infoboxIndex +
        '">Infobox Visibility</label><input type="range" min="0" max="1" step="0.001" value="' +
        parseFloat(infobox.infoboxVisibility) +
        '" class="slider" id="infobox-opacity-input-' +
        infobox.infoboxIndex +
        '" oninput="opacityInfobox(' +
        infobox.infoboxIndex +
        ');"></div><div class="form-group" id="infobox-bg-banner-color-' +
        infoboxIndex +
        '"><label>Infobox Button Color</label><input class="jscolor pointer " id="infoboxBgBannerColor-' +
        infobox.infoboxIndex +
        '" value="#ff0000" style="width: 100%" onchange="changeInfoBannerBg(' +
        index +
        ')"></div></div></div></div>';
      ...
      if (nodes[nodes.indexOf(selectedNode)].infoboxes[index].infoFontColor) {
        var fontcolor = document.getElementById(
          "infoTextColor-" + infoboxIndex
        );
        fontcolor.style.backgroundColor =
          nodes[nodes.indexOf(selectedNode)].infoboxes[index].infoFontColor;
      }
      if (nodes[nodes.indexOf(selectedNode)].infoboxes[index].infoBgColor) {
        ...
        bgcolor.style.backgroundColor =
          nodes[nodes.indexOf(selectedNode)].infoboxes[index].infoBgColor;
      }
      if (
        nodes[nodes.indexOf(selectedNode)].infoboxes[index].infoBannerBgColor
      ) {
        var bannerbgcolor = document.getElementById(
          "infoboxBgBannerColor-" + infoboxIndex
        );
        bannerbgcolor.style.backgroundColor =
          nodes[nodes.indexOf(selectedNode)].infoboxes[index].infoBannerBgColor;
      }
      if (infobox.infoBgOpacity) {
        var opacity = document.getElementById(
          "descriptionBgOpacity-" + infoboxIndex
        );
        opacity.value = infobox.infoBgOpacity;
        var displayvalue = "(" + Math.round(opacity.value * 100) + "%)";
        document.getElementById(
          "ImageDescriptionRangeValue" + infoboxIndex
      ...
      $("#infobox-type-selection-" + infobox.infoboxIndex).val(
        infobox.infoboxType
      );
    ...
    var addInfobxButton =
      '<button onmouseenter="mouseEnter(\'Create a New Banner using this option. You can customize the Banner Settings through the Tab on the Left.\');" onmouseleave="mouseLeave()" type="button" class="btn btn-success float-right mb-2" onclick="addNewInfobox(false)">New Banner</button>';
...
function addTeleportCards(selectedPlace, selectedNewTeleportID) {
  ...
  if (selectedPlace) {
    $("#teleport-place-name").text(
      "Customize teleports for " + selectedPlace.placeName
    );
    var index = 0;
    teleportIndex = 0;
    selectedPlace.teleports.forEach((teleport) => {
      teleport.teleportIndex = teleportIndex;
      teleport.teleportID = "teleport-" + teleportIndex;
      var teleportCollapseCard =
        '<div class="card" id="telport-' +
        teleport.teleportIndex +
        '"><div class="card-header" id="heading' +
        teleport.teleportIndex +
        '"><h3 class="mb-0"><button class="btn btn-link" data-toggle="collapse" data-target="#collapse-teleport-' +
        teleport.teleportID +
        '" aria-expanded="true" aria-controls="collapse-teleport-' +
        teleport.teleportID +
        '" onclick="setTeleportSelected(' +
        teleport.teleportIndex +
        ')" ><span id="teleport-collapse-button-' +
        index +
        '" >' +
        teleport.name +
        '</span></button><i class="fa fa-trash float-right trash-icon-position" onmouseenter="mouseEnter(\'Remove this Teleport.\');" onmouseleave="mouseLeave()" aria-hidden="true" onclick="setTeleportAndRemove(' +
        teleport.teleportIndex +
        ')"></i></h3></div><div id="collapse-teleport-' +
        teleport.teleportID +
        '" class="collapse" aria-labelledby="heading' +
        teleport.teleportIndex +
        '" data-parent="#teleport-accordion"><div class="card-body"><div class="form-group"><label for="teleport-name-input-' +
        teleportIndex +
        '">Name</label><input type="text" placeholder="Teleport name here." class="form-control input_box_trasparent"  id="teleport-name-input-' +
        teleportIndex +
        '" value="' +
        teleport.name +
        '" onchange="saveTeleportNameOnBlur(' +
        index +
        ", '" +
        teleport.teleportID +
        '\')" maxlength="25"></div><div class="form-group"><label for="teleport-focus-input-' +
        teleportIndex +
        '">Focus Text</label><textarea maxlength="150" placeholder="Focus Text here." class="form-control input_box_trasparent" id="teleport-text-input-' +
        teleportIndex +
        '" value="' +
        teleport.focusText +
        '" onchange="saveTeleportFocusTextOnBlur(' +
        index +
        ",'" +
        teleport.teleportID +
        "')\">" +
        teleport.focusText +
        '</textarea></div><div class="form-group"><label for="teleport-room-selection-' +
        teleport.teleportID +
        '">Teleport to</label><select class="form-control input_box_trasparent" id="teleport-room-selection-' +
        teleport.teleportID +
        '" onchange="setTeleportToOnChange(\'' +
        teleport.teleportID +
        "'," +
        index +
        ')"></select></div><div class="form-group"><label for="teleport-size-input-' +
        teleportIndex +
        '">Teleport Size</label><input type="range" min="1" max="10" step="0.01" value="' +
        parseInt(teleport.teleportSize) +
        '" class="slider" id="teleport-size-input-' +
        teleportIndex +
        '" oninput="resizeTeleport(' +
        teleportIndex +
        ",'" +
        teleport.teleportID +
        '\');"></div><div class="form-group"><label for="teleport-opacity-input-' +
        teleportIndex +
        '">Teleport Visibility</label><input type="range" min="0" max="1" step="0.001" value="' +
        teleport.teleportVisibility +
        '" class="slider" id="teleport-opacity-input-' +
        teleportIndex +
        '" oninput="opacityTeleport(' +
        teleportIndex +
        ",'" +
        teleport.teleportID +
        "');\"></div></div></div></div>";
      ...
      jscolor.installByClassName("jscolor");
      // if (
      //   nodes[nodes.indexOf(selectedNode)].teleports[teleportIndex]
      //     .teleportBannerBgColor
      // ) {
      //   var teleportBgcolor = document.getElementById(
      //     "teleportBgBannerColor-" + teleportIndex
      //   );
      //   teleportBgcolor.style.backgroundColor =
      //     nodes[nodes.indexOf(selectedNode)].teleports[
      //       teleportIndex
      //     ].teleportBannerBgColor;
      // }
    ...
    var addTeleportButton =
      '<button onmouseenter="mouseEnter(\'Add a new Teleport using this option. You can customize the navigation action for this Teleport through the Teleport Tab on the left.\');" onmouseleave="mouseLeave()" type="button" class="btn btn-success float-right mb-2 " onclick="addNewTeleport(false)">New Teleport</button>';
...
function toggleMapEditorLabel() {
  if ($("#graph").hasClass("hidden")) {
    ...
    $("#freezeView").removeClass("disabled");
    mapLable =
      "Open the Map View of the Experience. You can create a holastic map view of the Places that your Experience currently has. Each of the Teleport denotes an Arrow tail from the Place Arrow head denotes which place it leads to.";
...
function addTeleports(teleportsArray) {
  ...
  if (teleportsArray) {
    teleportsArray.forEach((teleport) => {
      teleport.teleportIndex = teleportIndex;
      teleport.teleportID = "teleport-" + teleportIndex;
      var teleportInputColor = document.getElementById(
        "teleportBgBannerColor-" + teleportIndex
      );
      var newTeleport = document.createElement("a-sphere");
      // if (teleport.teleportBannerBgColor) {
      //   newTeleport.setAttribute(
      //     "material",
      //     "color",
      //     teleport.teleportBannerBgColor
      //   );
      //   // teleportInputColor.style.backgroundColor=teleport.teleportBannerBgColor;
      // } else {
      newTeleport.setAttribute("material", "color", "#0f0");
      // teleportInputColor.style.backgroundColor ="#0f0";
      // }
      ...
      newTeleport.setAttribute(
        "event-set__1",
        "_event: dragstart; material.opacity: 0.2"
      );
      newTeleport.setAttribute(
        "event-set__1",
        "_event: dragend; material.opacity: 1"
      );
      ...
      newTeleportNamePlane.setAttribute(
        "material",
      ...
      newTeleportName.setAttribute(
        "position",
        "0 " + (-teleport.teleportSize - 2) + " 0"
      );
      teleportIndex++;
    });
  }
...
function addInfoboxes(infoboxesArray) {
  ...
  if (infoboxesArray) {
    infoboxesArray.forEach((infobox) => {
      ...
      newInfoboxNamePlane.setAttribute(
        "material",
        "transparent:true;opacity:0.03"
      );
      ...
      newInfoboxName.setAttribute(
        "position",
        "0 " + (-infobox.infoboxSize - 2) + " 0"
      );
      infoboxIndex++;
    });
  }
...
function addNewTeleport(flag) {
  ...
  //   );
  // } else {
  newTeleport.setAttribute("color", "#0f0");
  // teleportInputColor.setAttribute("value","#0f0");
  // }
...
function removeSkyAudio(placeIndex) {
  ...
  for (i = 0; i < nodes.length; i++) {
    if (nodes[i].placeIndex === placeIndex) {
      ...
      nodes[i].placeImageName = $(
          "#place-image-path-" + selectedNode.placeIndex
        )
        .text()
        .split("/")
        .pop();
      ...
      nodes[i].placeAudioName = $(
          "#place-audio-path-" + selectedNode.placeIndex
        )
        .text()
        .split("/")
        .pop();
      ...
      nodes.forEach((node) => {
        node.teleports.forEach((teleport) => {
          if (parseInt(teleport.teleportToPlaceIndex) === nodes[i].placeIndex) {
            ...
            teleportToPlace = $(
              "#place-name-input-" + selectedNode.placeIndex
            ).val();
          }
        });
      });
      restart();
      var removedAudio = document.getElementById("place-audio-preview-" + selectedNode.placeIndex);
      removedAudio.setAttribute("src", "");
      // addPlaceCards(nodes);
      // $('#place-accordion').find('.collapse').collapse('hide');
...
function toggleInfoboxType(type, infoboxIndex) {
  if (type === "Image") {
    ...
    $("#infobox-video-group-" + infoboxIndex).prop("hidden", true);
    $("#infobox-video-preview-div-" + infoboxIndex).prop("hidden", true);
    ...
    $("#infobox-audio-preview-div-" + infoboxIndex).prop("hidden", false);
  } else if (type === "Video") {
    $("#infobox-text-group-" + infoboxIndex).prop("hidden", true);
    $("#infobox-text-color-" + infoboxIndex).prop("hidden", true);
    $("#infobox-bg-opacity-" + infoboxIndex).prop("hidden", true);
    ...
    $("#infobox-image-group-" + infoboxIndex).prop("hidden", true);
    $("#infobox-image-preview-div-" + infoboxIndex).prop("hidden", true);
    $("#infobox-video-group-" + infoboxIndex).prop("hidden", false);
    $("#infobox-video-preview-div-" + infoboxIndex).prop("hidden", false);
    $("#infobox-audio-group-" + infoboxIndex).prop("hidden", true);
    $("#infobox-audio-preview-div-" + infoboxIndex).prop("hidden", true);
  ...
  nodes[nodes.indexOf(selectedNode)].infoboxes[infoboxIndex].infoboxType = $(
    "select#infobox-type-selection-" + infoboxIndex
  ).val();
}
...
function addInfoboxVideo(infoboxIndex, selectedPlace) {
  uploadVideo("Infobox", infoboxIndex, selectedPlace);
}
...
function setPlaceAndRemove(placeIndex) {
  ...
  if (placeIndex >= 0 && nodes.length > 1) {
    nodes.forEach((node) => {
      if (node.placeIndex === placeIndex) selectedNode = node;
    });
    nodes.splice(nodes.indexOf(selectedNode), 1);
    nodes.forEach((node) => {
      node.teleports.forEach((teleport) => {
...
function addPlaceSkyToJSON(placeIndex) {
  for (i = 0; i < nodes.length; i++) {
    if (nodes[i].placeIndex === placeIndex) {
      ...
      $(".graph-button").addClass("hidden-label");
      $(".scene-button").removeClass("hidden-label");
      toggleToolbarPlaceButtons(selectedNode, false);
      loadAscene(selectedNode);
      addInfoboxCards(selectedNode);
      addTeleportCards(selectedNode);
      break;
...
function setNodeSelected(index) {
  if (
    ...
    selectedNode == nodes[index] &&
    $("#toggleMapEditor").hasClass("disabled")
  ) {
    selectedNode = null;
    restart();
    addInfoboxCards(selectedNode);
...
function saveInfoboxNameOnBlur(index) {
  if (selectedNode) {
    ...
    nodes[nodes.indexOf(selectedNode)].infoboxes[
      index
    ].infoboxName = infoboxNewName;
...
function saveInfoboxTextOnBlur(index, infoboxID) {
  if (selectedNode) {
    nodes[nodes.indexOf(selectedNode)].infoboxes[index].text = $(
      "textarea#infobox-text-input-" + index
    ).val();
    loadAscene(selectedNode);
    // addInfoboxCards(selectedNode);
    // $('#infobox-accordion').find('.collapse').collapse('hide');
...
function changeInfoBannerBg(infoboxIndex) {
  var infoboxButtonColor = document.querySelector(
    "a-sphere#infobox-" + infoboxIndex
  );
  infoboxButtonColor.setAttribute(
    "material",
    "color",
    $("#infoboxBgBannerColor-" + infoboxIndex).val()
  );
  nodes[nodes.indexOf(selectedNode)].infoboxes[
    infoboxIndex
  ].infoBannerBgColor = parseFloat(
    $("#infoboxBgBannerColor-" + infoboxIndex).val()
  );
  if (selectedNode) {
    var infoBannerBgColor = $(
      "input#infoboxBgBannerColor-" + infoboxIndex
    ).val();
    if (infoBannerBgColor.includes("#")) {
      nodes[nodes.indexOf(selectedNode)].infoboxes[
        infoboxIndex
      ].infoBannerBgColor = infoBannerBgColor;
    } else {
      nodes[nodes.indexOf(selectedNode)].infoboxes[
        infoboxIndex
      ].infoBannerBgColor = "#" + infoBannerBgColor;
...
function changeTeleportBannerBg(teleportIndex, teleportID) {
  // var teleportButtonColor = document.querySelector(
  //   "a-sphere#" + teleportIndex
  // );
  var teleportButtonColor = document.querySelector("a-sphere#" + teleportID);
  teleportButtonColor.setAttribute(
    "material",
    "color",
    $("#teleportBgBannerColor-" + teleportIndex).val()
  );
  nodes[nodes.indexOf(selectedNode)].teleports[
    teleportIndex
  ].teleportBannerBgColor = parseFloat(
    $("#teleportBgBannerColor-" + teleportIndex).val()
  );
  if (selectedNode) {
    var teleportBannerBgColor = $(
      "input#teleportBgBannerColor-" + teleportIndex
    ).val();
    if (teleportBannerBgColor.includes("#")) {
      nodes[nodes.indexOf(selectedNode)].teleports[
        teleportIndex
      ].teleportBannerBgColor = teleportBannerBgColor;
    } else {
      nodes[nodes.indexOf(selectedNode)].teleports[
        teleportIndex
      ].teleportBannerBgColor = "#" + teleportBannerBgColor;
...
function changeOpacity(event, index) {
  if (selectedNode) {
    ...
    document.getElementById(
      "ImageDescriptionRangeValue" + index
    ).innerText = displayvalue;
    loadAscene(selectedNode);
  }
}
...
function saveInfoboxImageOnChange(selectedInfobox) {
  if (
    ...
    selectedInfobox != undefined
  ) {
    nodes[nodes.indexOf(selectedNode)].infoboxes[selectedInfobox].imageName = $(
        "#infobox-image-path-" + selectedInfobox
      )
      .text()
      .split("/")
      .pop();
    nodes[nodes.indexOf(selectedNode)].infoboxes[selectedInfobox].imagePath = $(
      "#infobox-image-path-" + selectedInfobox
    ).text();
    loadAscene(selectedNode);
    // addInfoboxCards(selectedNode);
...
function saveInfoboxVideoOnChange(selectedInfobox) {
  if (
    selectedNode != null &&
    selectedNode != undefined &&
    selectedInfobox != null &&
    ...
    nodes[nodes.indexOf(selectedNode)].infoboxes[selectedInfobox].videoName = $(
        "#infobox-video-path-" + selectedInfobox
      )
      .text()
      .split("/")
      .pop();
    nodes[nodes.indexOf(selectedNode)].infoboxes[selectedInfobox].videoPath = $(
      "#infobox-video-path-" + selectedInfobox
    ).text();
    loadAscene(selectedNode);
    // addInfoboxCards(selectedNode);
...
function saveInfoboxAudioOnChange(selectedInfobox) {
  if (
    ...
    nodes[nodes.indexOf(selectedNode)].infoboxes[selectedInfobox].audioName = $(
      "#infobox-audio-label-" + selectedInfobox
    ).text();
    nodes[nodes.indexOf(selectedNode)].infoboxes[selectedInfobox].audioPath = $(
      "#infobox-audio-path-" + selectedInfobox
    ).text();
    loadAscene(selectedNode);
  }
}
...
function saveTeleportFocusTextOnBlur(index, teleportID) {
  if (selectedNode) {
    nodes[nodes.indexOf(selectedNode)].teleports[index].focusText = $(
      "textarea#teleport-text-input-" + index
    ).val();
    loadAscene(selectedNode);
  }
}
...
function setTeleportToOnChange(teleportID, index) {
  if (selectedNode) {
    ...
    var teleportToID = parseInt(
      $("select#teleport-room-selection-" + teleportID).val()
    );
    nodes.forEach((node) => {
      if (node.placeIndex == teleportToID) teleportNode = node;
    });

    if (teleportToID >= 0) {
      nodes[nodes.indexOf(selectedNode)].teleports[
        index
      ].teleportToPlaceIndex = teleportToID;
      nodes[nodes.indexOf(selectedNode)].teleports[index].teleportToPlace =
        teleportNode.placeName;
      nodes[nodes.indexOf(selectedNode)].teleports[index].teleportToPlaceImage =
        teleportNode.placeSky;
...
function setInfoboxSelected(infoboxIndex) {
  if (
    ...
    selectedHotspot = selectedHotspot =  $("a-sphere#infobox-"+infoboxIndex)[0];
    $("#toggleMapEditor").removeClass("disabled");
    $("#graph").addClass("hidden");
    $("#scene").removeClass("hidden");
    toggleMapEditorLabel();
...
function resizeInfobox(infoboxIndex) {
  ...
  nodes[nodes.indexOf(selectedNode)].infoboxes[
    infoboxIndex
  ].infoboxSize = newInfoboxSize;
  // var infoboxPosition = infobox.getAttribute('position');
  var teleportText = document.querySelector(
    "#text-" +
    nodes[nodes.indexOf(selectedNode)].infoboxes[infoboxIndex].infoboxID +
    " > a-text"
  );
...
function opacityInfobox(infoboxIndex) {
  ...
  infobox.setAttribute(
    "opacity",
    $("#infobox-opacity-input-" + infoboxIndex).val()
  );
  nodes[nodes.indexOf(selectedNode)].infoboxes[
    infoboxIndex
  ].infoboxVisibility = parseFloat(
    $("#infobox-opacity-input-" + infoboxIndex).val()
  );
}

function resizeTeleport(teleportIndex, teleportID) {
  var newTeleportSize = parseInt(
    $("#teleport-size-input-" + teleportIndex).val()
  );
  nodes[nodes.indexOf(selectedNode)].teleports[
    teleportIndex
  ].teleportSize = newTeleportSize;
...
function opacityTeleport(teleportIndex, teleportID) {
  ...
  teleport.setAttribute(
    "opacity",
    $("#teleport-opacity-input-" + teleportIndex).val()
  );
  nodes[nodes.indexOf(selectedNode)].teleports[
    teleportIndex
  ].teleportVisibility = parseFloat(
    $("#teleport-opacity-input-" + teleportIndex).val()
  );
}
